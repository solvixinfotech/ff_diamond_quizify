...
import { Loader2, User, LogIn, Globe } from "lucide-react";
...
const Login = () => {
  ...
  const { loginWithFreeFire } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await loginWithFreeFire(freeFireId.trim(), region);
      ...
      if (error.message.includes("not found")) {
        errorMessage = "Free Fire ID not found. Please sign up first.";
        ...
        <Card className="w-full max-w-md border-border bg-card">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">
              Log in with your Free Fire ID to continue your quiz journey
            </CardDescription>
          </CardHeader>
          <CardContent>
